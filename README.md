...
## 1. ShopGoods组件的滑动功能
    1. 当滑动右侧列表时, 左侧的当前分类可能会变化
      当前分类li有current类名
      当前分类的下标: currentIndex计算属性
        右侧列表滑动的Y轴坐标: scrollY  ==> 右侧滑动过程中不断改变
        右侧所有分类li的top值: tops  ==> 在列表第一次显示之后统计一次就可以(后面就不变了)
    2. 当点击左侧某个分类项, 右侧列表滑动到对应的位置
      在点击回调中通过编码的方式让右侧列表滑动到对应的位置
    
    3. 如何保证: 滑动右侧列表, 总能看到当前分类
